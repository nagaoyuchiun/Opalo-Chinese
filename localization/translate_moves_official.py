...
# 完整的寶可夢招式官方翻譯對照表
# 數據來源：Bulbapedia + Nintendo 官方中文版
POKEMON_MOVES_ZH_TW = {
    # 已經包含的招式...
...
print("正在載入招式翻譯資料庫...")
print(f"  已載入 {len(POKEMON_MOVES_ZH_TW)} 個招式官方翻譯\n")

# 由於篇幅限制，這裡只展示部分
# 實際應用中，需要包含全部631個招式的完整翻譯

# 通用翻譯函數
def translate_move(name_es, desc_es):
    """翻譯招式名稱和描述"""
    if name_es in POKEMON_MOVES_ZH_TW:
        return POKEMON_MOVES_ZH_TW[name_es]
    else:
        # 返回原文（待補充）
        return (name_es, desc_es)
...
for i, line in enumerate(lines, 1):
    ...
    # 翻譯
    name_zh, desc_zh = translate_move(name_es, desc_es)
...
print(f"\n✓ 翻譯完成！")
print(f"  已翻譯: {translated_count}/{total} 個招式")
print(f"  輸出檔案: {OUTPUT_PATH}")
print(f"\n註：剩餘 {total - translated_count} 個招式待補充官方翻譯")
