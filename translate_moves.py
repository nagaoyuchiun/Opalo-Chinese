...
# 官方寶可夢招式中文譯名對照表（基於 Nintendo 台灣官方翻譯）
OFFICIAL_MOVES = {
    "Megacuerno": "超級角擊",
    "Al Ataque": "攻擊指令",
    "Zumbido": "蟲鳴",
...
# 建立術語替換映射
term_map = {}
for category_data in glossary['categories'].values():
    if 'terms' in category_data:
    ...
    if 'subcategories' in category_data:
        for subcat_data in category_data['subcategories'].values():
            if 'terms' in subcat_data:
                for term in subcat_data['terms']:
                    if 'es' in term and 'zh_TW' in term:
                        term_map[term['es']] = term['zh_TW']
...
def translate_description(text):
    """翻譯招式描述"""
    # 先替換術語表中的詞彙
    for es, zh in term_map.items():
        # 使用單詞邊界匹配避免誤替換
        text = re.sub(r'\b' + re.escape(es) + r'\b', zh, text, flags=re.IGNORECASE)
    
    # 常見動詞和短語翻譯
    replacements = {
        "Violenta embestida con cuernos imponentes": "用堅硬的角猛烈撞擊對手",
        "El usuario llama a sus amigos para que ataquen al rival": "命令手下發動攻擊",
        "Suele ser crítico": "容易擊中要害",
        "El movimiento de las alas crea una onda sónica dañina": "透過翅膀振動產生破壞性音波",
        "También puede disminuir la Defensa Especial del objetivo": "有時會降低對手的特防",
        "Cruza las guadañas o las garras para atacar al rival como si fueran unas tijeras": "將鐮刀或爪子像剪刀般交叉劈向對手",
        "Ataca con un rayo de luz siniestro": "發射不可思議的光線攻擊",
        "Puede confundir al objetivo": "有時會使對手混亂",
        "Tras atacar, vuelve a toda prisa para dar paso a otro Pokémon del equipo": "攻擊後迅速返回並替換其他寶可夢",
        "El usuario se hace una bola y arrolla al objetivo con su cuerpo": "蜷縮成球狀滾動攻擊對手",
        "Puede hacerlo retroceder": "有時會使對手畏縮",
        "Pica al rival": "啃咬對手",
        "Si el adversario lleva una Baya, el agresor se la come y se beneficia de su efecto": "若對手攜帶樹果，可以吃掉並獲得其效果",
        "Fuerte viento con polvo de escamas": "用鱗粉颳起強風攻擊",
        "Puede subir todas las características de quien lo usa": "有時會提升使用者所有能力",
        "El usuario contraataca": "進行反擊",
        "Además baja el Ataque Especial de los oponentes": "降低對手的特攻",
        "Clava aguijones al rival dos veces": "用針刺向對手兩次",
        "Puede envenenar": "有時會使對手中毒",
        ...
        "El usuario emplea la fuerza del objetivo para atacarlo": "利用對手的力量攻擊",
        "Cuanto mayor es el atq. del objetivo, más daño provoca": "對手攻擊力越高傷害越大",
        "Ataca al objetivo con una onda siniestra": "用不祥的波動攻擊對手",
        "Puede bajar su Precisión": "有時會降低命中率",
        "Tritura con afilados colmillos y puede bajar la Defensa del objetivo": "用尖牙咬碎對手，有時會降低防禦",
        "Libera una horrible aura llena de malos pensamientos y puede hacer retroceder al objetivo": "釋放充滿惡意的恐怖波動，有時會使對手畏縮",
        "Permite atacar primero": "先制攻擊",
        "Falla si el objetivo no está preparando ningún ataque": "若對手沒有使用攻擊招式則會失敗",
        "Ataca al objetivo a la primera oportunidad": "趁對手不備發動攻擊",
        ...
        "Ataca y le quita al objetivo el objeto que lleva": "攻擊對手並奪取其攜帶的道具",
        "Si el agresor lleva un objeto": "若自己攜帶道具則無法奪取",
        "Impide al objetivo usar el objeto que lleva durante el combate": "拍落對手的道具使其無法使用",
        "Ataque de todo el equipo Pokémon": "同行寶可夢全體攻擊",
        "Cuantos más haya, más veces ataca": "寶可夢越多攻擊次數越多",
        "Lanza contra el objetivo el objeto que lleva": "將自己的道具丟向對手",
        "La fuerza del ataque y su efecto varían según el objeto": "威力和效果依道具而定",
        ...
        "Estimula su cerebro pensando en cosas malas": "思考壞事刺激大腦",
        "Aumenta considerablemente el Ataque Especial": "大幅提升特攻",
...
for line in lines:
    ...
    if match:
        ...
        # 找到描述部分（在引號內）
        desc_match = re.search(r'"([^"]*)"', rest_with_desc)
        if desc_match:
            original_desc = desc_match.group(1)
            translated_desc = translate_description(original_desc)
            # 替換描述
            new_rest = rest_with_desc.replace(f'"{original_desc}"', f'"{translated_desc}"')
        else:
            new_rest = rest_with_desc
        
        # 重組行
        new_line = f'{id_num},{internal},{translated_name},{new_rest}{remainder}\n'
